...
namespace NBXplorer.Backends.Postgres
{
	public class DbConnectionHelper : IDisposable, IAsyncDisposable
	{
		...
		record NewOutRaw(string tx_id, long idx, string script, long value, string asset_id);
		record NewInRaw(string tx_id, long idx, string spent_tx_id, long spent_idx);
