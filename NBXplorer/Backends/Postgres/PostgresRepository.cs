...
namespace NBXplorer.Backends.Postgres
{
	...
	public class PostgresRepository : IRepository
	{
		...
		{
			var hash = nbxv1_get_descriptor_id(Network.CryptoCode, strategy.ToString(), derivationFeature.ToString());
		...
		{
			var hash = nbxv1_get_wallet_id(Network.CryptoCode, strategy.ToString());
		...
		internal static string nbxv1_get_wallet_id(string cryptoCode, string addressOrDerivation)
		{
			return Encoders.Base64.EncodeData(Hashes.SHA256(new UTF8Encoding(false).GetBytes($"{cryptoCode}|{addressOrDerivation}")), 0, 21);
		}
		internal static string nbxv1_get_descriptor_id(string cryptoCode, string strategy, string feature)
		{
		...
		WalletKey GetWalletKey(IDestination destination)
		{
			...
			var hash = nbxv1_get_wallet_id(Network.CryptoCode, address.ToString());
			JObject m = new JObject();
		...
		record DescriptorScriptInsert(string code, string descriptor, int idx, string script, string metadata, string addr, string walletid);
		public async Task<int> GenerateAddresses(DerivationStrategyBase strategy, DerivationFeature derivationFeature, GenerateAddressQuery query = null)
		{
			...
			await connection.ExecuteAsync(
				...
				"INSERT INTO descriptors_scripts VALUES (@code, @descriptor, @idx, @script, @metadata::JSONB) ON CONFLICT DO NOTHING;"
				, linesScriptpubkeys);
			return toGenerate;
		}
		...
		record KeyInfoInsert(string code, string descriptor, string script, string address, long? idx, string walletid, string metadata);
		...
		internal async Task SaveKeyInformations(DbConnection connection, KeyPathInformation[] keyPathInformations)
		{
			...
			if (hdParameters.Count > 0)
				await connection.ExecuteAsync(
				"INSERT INTO scripts VALUES (@code, @script, @address) ON CONFLICT DO NOTHING;" +
				"INSERT INTO descriptors_scripts VALUES (@code, @descriptor, @idx, @script, @metadata::JSONB, 't') ON CONFLICT (code, descriptor, idx) DO UPDATE SET used='t';"
				, hdParameters);

			var singleParameters = inserts.Where(i => i.descriptor is null).ToList();
			if (singleParameters.Count > 0)
				await connection.ExecuteAsync(
					"INSERT INTO scripts VALUES (@code, @script, @address) ON CONFLICT DO NOTHING;" +
					"INSERT INTO wallets_scripts VALUES (@code, @script, @walletid) ON CONFLICT DO NOTHING;", singleParameters);
		}
