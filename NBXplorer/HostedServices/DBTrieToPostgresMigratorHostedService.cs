...
namespace NBXplorer.HostedServices
{
	public class DBTrieToPostgresMigratorHostedService : IHostedService
	{
		...
		record InsertOuts(string code, string tx_id, long idx, string script, long value, string asset_id, DateTime seen_at);
		record InsertIns(string code, string tx_id, long idx, string spent_tx_id, long spent_idx, DateTime seen_at);
		private async Task Migrate(NBXplorerNetwork network, Repository legacyRepo, PostgresRepository postgresRepo, ILogger logger, CancellationToken cancellationToken)
		{
			...
			if (!progress.EventsMigrated)
			{
				if (!ExplorerConfiguration.NoMigrateEvents)
				{
					using (var tx = await conn.BeginTransactionAsync())
					{
						...
						long lastEventId = -1;
						nextbatch:
						var batch = await legacyRepo.GetEvents(lastEventId, 200);
						if (batch.Count > 0)
						{
				...
				if (!ExplorerConfiguration.NoMigrateRawTxs)
				{
					...
					await foreach (var row in savedTxsTable.Enumerate())
					{
						...
						using (row)
						{
							...
							{
								await InsertTransactions(conn, batchTxs, batchBlocksTxs);
			...
			if (!progress.TrackedTransactionsMigrated)
			{
				...
				List<InsertOuts> outputs = new List<InsertOuts>(1000);
				var batchBlocksTxs = new List<UpdateBlockTransaction>(1000);
				...
				await foreach (var row in savedTxsTable.Enumerate())
				{
					...
					using (row)
					{
						...
						{
							batchBlocksTxs.Add(new UpdateBlockTransaction(network.CryptoCode, tt.TransactionHash.ToString(), tt.BlockHash.ToString()));
						}
						if (processedTxs.Add((tt.TransactionHash, trackedSourceHash)))
						{
							batchTxs.Add(new UpdateTransaction(network.CryptoCode, tt.TransactionHash.ToString(), null, tt.FirstSeen.UtcDateTime));
							foreach (var o in tt.GetReceivedOutputs())
							{
								...
								outputs.Add(new InsertOuts(
									...
									value,
									assetId,
									tt.FirstSeen.UtcDateTime));
							}
						}
						if (batchTxs.Count >= 1000)
						{
							await InsertTransactions(conn, batchTxs, batchBlocksTxs);
							await conn.ExecuteAsync("INSERT INTO outs VALUES (@code, @tx_id, @idx, @script, @value, @asset_id) ON CONFLICT DO NOTHING;", outputs);
							outputs.Clear();
							batchTxs.Clear();
							batchBlocksTxs.Clear();
							processedTxs.Clear();
			...
			if (!progress.TrackedTransactionsInputsMigrated)
			{
				...
				List<InsertIns> filteredBatch = new List<InsertIns>(1000);
				...
				await foreach (var row in savedTxsTable.Enumerate())
				{
					...
					using (row)
					{
						...
						foreach (var o in tt.SpentOutpoints)
						{
							...
							if (filteredBatch.Count >= 1000)
							{
								await InsertInsMatches(conn, filteredBatch);
							}
						}
					}
				}
		...
		private static async Task InsertInsMatches(System.Data.Common.DbConnection conn, List<InsertIns> filteredBatch)
		{
			await conn.ExecuteAsync("INSERT INTO ins VALUES (@code, @tx_id, @idx, @spent_tx_id, @spent_idx) ON CONFLICT DO NOTHING", filteredBatch);
			filteredBatch.Clear();
		}

		private static async Task FindInsMatches(NBXplorerNetwork network, System.Data.Common.DbConnection conn, List<InsertIns> batch, List<InsertIns> filteredBatch)
		{
		...
		private static async Task InsertTransactions(System.Data.Common.DbConnection conn, List<UpdateTransaction> batchTxs, List<UpdateBlockTransaction> batchBlocksTxs)
		{
			await conn.ExecuteAsync(
				...
				"WHERE (t.seen_at > EXCLUDED.seen_at) OR (t.raw IS NULL AND EXCLUDED.raw IS NOT NULL);", batchTxs);
			await conn.ExecuteAsync(
